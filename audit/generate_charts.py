"""
Generate charts for the LIME audit article.

Usage:
    python -m audit.generate_charts

Produces:
    audit/results/chart_jaccard_by_input.png
    audit/results/chart_faithfulness_scatter.png
"""
...
from audit.config import RESULTS_DIR, STABILITY_CSV_PATH, DELETION_CSV_PATH, STABILITY_THRESHOLD_JACCARD
...
def load_deletion():
    rows = []
    with open(DELETION_CSV_PATH, "r", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
...
def chart_jaccard(stability_rows):
    ...
    ax.axhline(y=STABILITY_THRESHOLD_JACCARD, color="#cc0000", linestyle="--",
               linewidth=1.5, label=f"Threshold ({STABILITY_THRESHOLD_JACCARD})")

    input21_idx = next(i for i, r in enumerate(sorted_rows) if r["input_id"] == 21)
    ax.annotate(
        'Input 21\n"best product I have\never purchased"',
        xy=(input21_idx, sorted_rows[input21_idx]["mean_jaccard"]),
        xytext=(input21_idx + 3, 0.25),
        fontsize=8,
        arrowprops=dict(arrowstyle="->", color="#333"),
        bbox=dict(boxstyle="round,pad=0.3", facecolor="#fff3cd", edgecolor="#333"),
    )
    ...
    fig.tight_layout()
    path = os.path.join(RESULTS_DIR, "chart_jaccard_by_input.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"Saved: {path}")


def chart_faithfulness(deletion_rows):
    cat_deltas = {}
    ...
    ax1.set_xticks(x)
    ax1.set_xticklabels([CATEGORY_LABELS[c] for c in cats_present], rotation=30, ha="right", fontsize=8)
    ...
    ax2.set_xticks(x)
    ax2.set_xticklabels([CATEGORY_LABELS[c] for c in cats_present], rotation=30, ha="right", fontsize=8)
    ax2.legend(fontsize=8)
    ax2.set_ylim(0, 110)

    fig.tight_layout()
    path = os.path.join(RESULTS_DIR, "chart_faithfulness_bars.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"Saved: {path}")
...
def main():
    ...
    deletion = load_deletion()
    chart_jaccard(stability)
    chart_faithfulness(deletion)
    print("Done.")
